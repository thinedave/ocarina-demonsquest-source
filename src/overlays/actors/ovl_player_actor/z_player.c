...
#include "overlays/actors/ovl_Savespot/z_savespot.h"
...
#include "overlays/actors/ovl_XPDrop/z_xpdrop.h"

u32 Player_PerfectTime = 0;
u32 Player_PerfectResetTimer = 0;
u32 Player_StoredDoubleDamageTimer = 0;
...
void Player_CheckShieldDurability(Player* this, PlayState* play, f32 dmg);
void Player_CheckShieldBreak(u8 durability, PlayState* play, Player* this);
bool findSaveSpot(Player* this, PlayState* play);
void activateSaveSpot(Player* this, PlayState* play);
...
static GetItemEntry sGetItemTable[] = {
    ...
    // GI_DEKU_NUTS_5
    GET_ITEM(ITEM_NONE, OBJECT_INVALID, 0, 0x34, 0x0C, CHEST_ANIM_SHORT),
    // GI_BOMBCHUS_10
...
s32 func_80834758(PlayState* play, Player* this) {
    ...
    if (!(this->stateFlags1 & (PLAYER_STATE1_22 | PLAYER_STATE1_23 | PLAYER_STATE1_29)) &&
        ...
        CHECK_BTN_ALL(sControlInput->cur.button, BTN_R) && play->stamina > 0) {
...
s32 func_80834B5C(Player* this, PlayState* play) {
    ...
    if (!CHECK_BTN_ALL(sControlInput->cur.button, BTN_R) || play->stamina <= 20) {
        func_80834894(this);
        return 1;
    } else if(CHECK_BTN_ALL(sControlInput->cur.button, BTN_R) && play->stamina > 0) {
    ...
    if ((this->unk_6AD != 0) || (play->transitionTrigger == TRANS_TRIGGER_START) ||
        ...
        *outYawTarget = sControlStickAngle;
        ...
        if (sControlStickMagnitude != 0.0f) {
            ...
            f32 lerp = *outSpeedTarget * 0.0213f;
            *outSpeedTarget = F32_LERP(0, speedCap, lerp);

            *outSpeedTarget -= (8.0f * floorPitchInfluence * floorPitchInfluence);
...
void Player_ChangePoise(PlayState* play, s8 amount) {
    play->poise = play->poise + amount;

    if(play->poise > 100) {
        play->poise = 100;

    }else if(play->poise < 0) {
        play->poise = 0;
...
void Player_ChangeStability(PlayState* play, s8 amount) {
    play->stability = play->stability + amount;

    if(play->stability > 100) {
        play->stability = 100;

    }else if(play->stability < 0) {
        play->stability = 0;
...
void func_80837C0C(PlayState* play, Player* this, s32 arg2, f32 arg3, f32 arg4, s16 arg5, s32 arg6) { //Player_TakeDamage
    LinkAnimationHeader* anim = NULL;
    LinkAnimationHeader** sp28;

    //func_8002F6D4(play, &this->actor, 2.0f, this->actor.world.rot.y*-1, 0.0f, 8);
...
void Player_CheckShieldBreak(u8 durability, PlayState* play, Player* this) {
    if(durability<=0) {
        Audio_PlaySfxGeneral(NA_SE_IT_MAJIN_SWORD_BROKEN, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                            &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
        Inventory_DeleteEquipment(play, EQUIP_TYPE_SHIELD);
        Message_StartTextbox(play, 0x305F, NULL);
        
        gSaveContext.save.info.playerData.shieldHasBroken = true;
...
void Player_CheckShieldDurability(Player* this, PlayState* play, f32 dmg) {
    if (this->currentShield == PLAYER_SHIELD_MIRROR || this->currentShield == PLAYER_SHIELD_NONE) {return;}

    u8 durability;

    Rand_Seed((u32)osGetTime());

    switch(this->currentShield) {
        case PLAYER_SHIELD_DEKU:
            Player_CheckShieldBreak(gSaveContext.save.info.playerData.shieldDurabilityDeku, play, this);
            gSaveContext.save.info.playerData.shieldDurabilityDeku = gSaveContext.save.info.playerData.shieldDurabilityDeku - CLAMP_MIN((Rand_Next()%5)+1, 1);

            if(gSaveContext.save.info.playerData.shieldDurabilityDeku>25) {gSaveContext.save.info.playerData.shieldDurabilityDeku = 0;}

            durability = gSaveContext.save.info.playerData.shieldDurabilityDeku;
            break;

        case PLAYER_SHIELD_HYLIAN:
            Player_CheckShieldBreak(gSaveContext.save.info.playerData.shieldDurabilityHylian, play, this);
            gSaveContext.save.info.playerData.shieldDurabilityHylian = gSaveContext.save.info.playerData.shieldDurabilityHylian - CLAMP_MIN((Rand_Next()%5)+1, 1);

            if(gSaveContext.save.info.playerData.shieldDurabilityHylian>50) {gSaveContext.save.info.playerData.shieldDurabilityHylian = 0;}

            durability = gSaveContext.save.info.playerData.shieldDurabilityHylian;
            break;
    ...
    Player_CheckShieldBreak(durability, play, this);
...
/*
MM equivalent to func_8083A4A8
...
void Player_OcarinaStart(Player* this, PlayState* play) {
    Player_ZeroSpeedXZ(this);
    func_8084B498(this);
...
s32 Player_ActionChange_13(Player* this, PlayState* play) {
    ...
    if ((this->unk_6AD != 0) && (func_808332B8(this) || (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) ||
        ...
        if (!Player_StartCsAction(play, this)) {
            if (this->unk_6AD == 4) {
                if(this->pullOcarina) {
                    Player_OcarinaStart(this, play);

                    this->pullOcarina = false;
                ...
                if (sp2C >= 0) {
                    if (sp2C == 0xC) {
                        ...
                        LinkAnimation_Change(play, &this->skelAnime, &gPlayerAnim_link_bottle_drink_demo_start, 1.0f, 35.0f, Animation_GetLastFrame(&gPlayerAnim_link_bottle_drink_demo_start), ANIMMODE_ONCE, -6.0f);
...
s32 Player_ActionChange_10(Player* this, PlayState* play) {
    ...
    if (CHECK_BTN_ALL(sControlInput->press.button, BTN_A) && play->stamina > 0 &&
        ...
        if (sp2C <= 0) {
            if (func_80833BCC(this)) {
                if (this->actor.category != ACTORCAT_PLAYER) {
                    ...
                    if ((Player_GetMeleeWeaponHeld(this) != 0) && Player_CanUpdateItems(this) && play->stamina > 0 /*&& play->stability > 25*/) {
...
static u8 sSprintStaminaTimer = 0;

s32 Player_ActionChange_6(Player* this, PlayState* play) {
    if(!func_80833B54(this) && (D_808535E0 == 0) && !(this->stateFlags1 & PLAYER_STATE1_23)) {
        if(CHECK_BTN_ALL(sControlInput->rel.button, BTN_A) && play->stamina > 0 && sSprintTimer < 5 && func_8083BC7C(this, play))
            return 1;
        else if(CHECK_BTN_ALL(sControlInput->press.button, BTN_A)) {
            if(findSaveSpot(this, play) == true)
                activateSaveSpot(this, play);
            else if(play->stamina > 0) {
    ...
    if(CHECK_BTN_ALL(sControlInput->cur.button, BTN_A) && play->stamina > 0) {
        ...
        if(sSprintTimer >= 5) {
            ...
            REG(45) = F32_LERP(REG(44), to, frac);

            if(sSprintStaminaTimer <= 0) {
                sSprintStaminaTimer = 8;
                Player_ChangeStamina(play, (slow ? -7 : 0));
    ...
    /*if (!func_80833B54(this) && (D_808535E0 == 0) && !(this->stateFlags1 & PLAYER_STATE1_23) &&
        ...
        if (findSaveSpot(this, play) == true) {
            ...
            if (Player_SprintTimer < 3 && func_8083BC7C(this, play)) {
                return 1;
            }
            if ((this->unk_837 == 0) && (this->heldItemAction >= PLAYER_IA_SWORD_MASTER)) {
                Player_UseItem(play, this, ITEM_NONE);
...
s32 func_808428D8(Player* this, PlayState* play) { //crouchstab
    if (!Player_IsChildWithHylianShield(this) && (Player_GetMeleeWeaponHeld(this) != 0) && sUseHeldItem && play->stamina > 0) {
        Player_ChangeStamina(play, -40);
...
void Player_InitCommon(Player* this, PlayState* play, FlexSkeletonHeader* skelHeader) {
    ...
    if (gSaveContext.saveReturnFlag > 0 && gSaveContext.save.info.savePoint != -1) {
        ...
        // link's position
        this->actor.home.pos = this->actor.world.pos = gSaveContext.save.info.saveRespawnPoint.pos;
        this->actor.home.rot.y = this->actor.world.rot.y = this->actor.shape.rot.y = gSaveContext.save.info.saveRespawnPoint.yaw;
        this->actor.params = gSaveContext.save.info.saveRespawnPoint.playerParams;

        // kill old transition actors I guess
        func_80096FD4(play, &play->roomCtx.curRoom);

        // load the room
        play->roomCtx.status = 0;
        func_8009728C(play, &play->roomCtx, gSaveContext.save.info.saveRespawnPoint.roomIndex);

        // load new transition actors...?
        func_800973FC(play, &play->roomCtx);

        // turn off the flag
        gSaveContext.saveReturnFlag = 0;
...
bool findSaveSpot(Player* this, PlayState* play) {
    Actor* save = Actor_FindNearby(play, &this->actor, ACTOR_SAVESPOT, ACTORCAT_ITEMACTION, 35.0f);
    if (save != NULL) {
        return true;
    }
    return false;
...
void activateSaveSpot(Player* this, PlayState* play) {
    Savespot* save = Actor_FindNearby(play, &this->actor, ACTOR_SAVESPOT, ACTORCAT_ITEMACTION, 35.0f);
    if (save != NULL) {
        Actor_PlaySfx(&this->actor, NA_SE_EV_TRE_BOX_FLASH); // 	NA_SE_EV_LINK_WARP;
        gSaveContext.save.info.savePoint = save->actor.params;
        gSaveContext.save.info.saveRespawnPoint.pos = save->actor.world.pos;
    ...
    if ((Message_GetState(&play->msgCtx) == TEXT_STATE_NONE) && (this->actor.category == ACTORCAT_PLAYER)) {
        ...
        if (!Player_InBlockingCsMode(play, this)) {
            if (this->stateFlags1 & PLAYER_STATE1_20) {
                ...
                } else if (!func_80833B54(this) && (D_808535E0 == 0) && !(this->stateFlags1 & PLAYER_STATE1_23) && !func_80833BCC(this) && findSaveSpot(this, play) == true) { // save do action
                    doAction = DO_ACTION_SAVE;
...
void Player_UpdateCamAndSeqModes(PlayState* play, Player* this) {
    ...
    if (this->actor.category == ACTORCAT_PLAYER) {
        ...
        if(REST_ISRESTING(restingState) && Play_CamIsNotFixed(play)) {
...
void Player_UpdateDemonsTimer(s8 *timer, s16 *stat) {
    *timer = *timer - 1;
...
void Player_UpdateCommon(Player* this, PlayState* play, Input* input) {
    ...
    //Player_UpdateDemonsTimer(&poiseTimer, &play->poise);
    Player_UpdateDemonsTimer(&staminaTimer, &play->stamina);
    //Player_UpdateDemonsTimer(&stabilityTimer, &play->stability);
    ...
    CHECK_BTN_ALL(sControlInput->cur.button, BTN_R)) {
        ...
        if(Player_PerfectTime == 0 && Player_CanPerfect && !Player_HasPerfected) {
            ...
            Player_PerfectResetTimer = 17446709;
            //Player_PlaySfx(&this->actor, NA_SE_IT_HAMMER_SWING);
            
        }

    } else if((play->shootingGalleryStatus == 0) && (this->currentShield != PLAYER_SHIELD_NONE) &&
    !CHECK_BTN_ALL(sControlInput->cur.button, BTN_R)) {
    ...
    f32 vSpeedScale = sWorldYawToTouchedWall * 0.00008f;

    if (!(this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) || vSpeedScale >= 1.0f) {
        this->unk_880 = R_RUN_SPEED_LIMIT / 100.0f;
    } else {
        f32 vSpeedLimit = (R_RUN_SPEED_LIMIT / 100.0f * vSpeedScale);
        this->unk_880 = vSpeedLimit;
...
void Player_Update(Actor* thisx, PlayState* play) {
    ...
    /*if (func_8084FCAC(this, play))*/ if(true) {
        if (gSaveContext.dogParams < 0) {
...
void Player_Action_8084BF1C(Player* this, PlayState* play) {
    ...
    this->skelAnime.playSpeed = (phi_f2 * phi_f0) * 3.0f;
        ...
        if (LinkAnimation_Update(play, &this->skelAnime) != 0) {
            ...
            if (sp84 != 0) {
                ...
                if (sp84 > 0) {
                    ...
                    temp_f0 = func_8083973C(play, this, &D_8085488C, &sp5C); // floor height
...
void Player_Action_8084EAC0(Player* this, PlayState* play) {
    if (LinkAnimation_Update(play, &this->skelAnime)) {
        ...
        if ((gSaveContext.healthAccumulator == 0) && (gSaveContext.magicState != MAGIC_STATE_FILL)) {
            //LinkAnimation_Change(play, &this->skelAnime, &gPlayerAnim_link_bottle_drink_demo_end, 1.0f, 28.0f, Animation_GetLastFrame(&gPlayerAnim_link_bottle_drink_demo_end), ANIMMODE_ONCE, 6.0f);
            LinkAnimation_Change(play, &this->skelAnime, &gPlayerAnim_link_bottle_drink_demo_end, 1.0f, 28.0f, Animation_GetLastFrame(&gPlayerAnim_link_bottle_drink_demo_end), ANIMMODE_ONCE, -6.0f);
...
s32 Player_ActionChange_7(Player* this, PlayState* play) {
    if (func_8083C6B8(play, this) == 0) {
        if (func_8083BB20(this) != 0 && play->stamina > 0 && play->swordOut == true) { // standard swing
...
void Player_Action_808502D0(Player* this, PlayState* play) { //swing weapon
