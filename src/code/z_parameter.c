...
void Health_GiveHearts(s16 hearts) {
    ...
    gSaveContext.save.info.playerData.permHealthCapacity += hearts * 0x10;
}
    ...
    if (gSaveContext.save.info.playerData.magicLevel != 0) {
        ...
        if (gSaveContext.magicState == MAGIC_STATE_METER_FLASH_2) {
            ...
            gSPTextureRectangle(OVERLAY_DISP++, R_MAGIC_FILL_X << 2, (magicMeterY + 2) << 2,
                                (R_MAGIC_FILL_X + gSaveContext.save.info.playerData.magic) << 2, (magicMeterY + 6) << 2, G_TX_RENDERTILE, 0,
                                0, 1 << 10, 1.6 * (1 << 10));
...
void Interface_SaveResting_Draw(PlayState* play) {
    ...
    switch(this->state) {
        case REST_STATE_IDLE:
            ...
            SaveResting_DrawStringShadowed(play, &gfx, levelUpText, 8, 20, textY, SAVEREST_GETR(0), SAVEREST_GETG(0), SAVEREST_GETB(0), 255, false, 1.0f);
        
            char exitText[] = "Exit";
            SaveResting_DrawStringShadowed(play, &gfx, exitText, 4, 20, textY + SAVEREST_LINESPACE, SAVEREST_GETR(1), SAVEREST_GETG(1), SAVEREST_GETB(1), 255, false, 1.0f);
